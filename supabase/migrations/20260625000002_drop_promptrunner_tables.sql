...
-- or the still-live old frontend would query dropped relations. That ordering is
-- the whole point: a drop is not reversible by re-deploying the code.
--
...
-- NOTE — manual prod follow-ups (not done here; they require superuser / Storage
-- API access, so no migration can perform them):
