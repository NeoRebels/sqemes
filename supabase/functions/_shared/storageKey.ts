 ...
 *
 * ⚠️ **This file has a twin: `lib/storageKey.ts`.** The Deno edge functions and the browser bundle
 * share no module system, so the rule exists twice — the same arrangement as
 * `_shared/injectionScan.ts` and `lib/injectionScan.ts`. `tests/unit/storageKey.test.ts` compares the
