...
// SQEM-378 — the library instruction now lives in ONE place and this is a pointer to it.
//
// ⛔ It used to be a full copy here, with a second, differently-worded copy inside
// `supabase/functions/mcp-server`. Two texts with one job, neither aware of the other, and only the
// server's copy had learned about personas.
//
// ⚠️ **Renamed from `MCP_SYSTEM_PROMPT`, and that is not cosmetic.** It governs the Sqemes chat too
// now (SQEM-373/378); a name saying "MCP" would have been the next thing to mislead somebody.
//
