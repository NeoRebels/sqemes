...
### 5b. Email confirmation — why it ships off, and when to turn it on

The bundle sets **`ENABLE_EMAIL_AUTOCONFIRM=true`** (`.env`), so a new account works immediately and
no confirmation link is required. **That is deliberate, not an oversight.**

Confirmation depends on a working mailer. If it were on by default, an operator who has not yet
configured SMTP would be locked out **by the very first account they create — their own** — with an
unusable login and nothing on screen explaining why. Since that is the one account you cannot
recover through the product, the default errs toward letting you in.

**Turn it on once SMTP works, if your instance is reachable by people you have not invited:**
...
then restart the stack. Verify by registering a throwaway address and confirming that sign-in is
refused until the link is clicked.

⚠️ **Check your mailer's rate limit before flipping this.** With confirmation required, an account
is unusable until its mail arrives — so a send that is throttled or dropped is a person who cannot
get in at all. Supabase's built-in SMTP is rate-limited and not meant for production use; configure
your own before relying on it.

If your instance is only reachable inside your own network and everyone on it is meant to have an
account, leaving autoconfirm on is a reasonable choice. Decide it — do not inherit it.
...
A failed connection reports the provider's own error, for example
*Connection failed: token_exchange (`invalid_client`)*. The full response is in the
`connector-oauth-callback` function log.

| Code | What it actually means |
|---|---|
| **`invalid_client`** | The client secret is wrong or expired. **Two traps, both common:** Microsoft's client secrets **expire** (6 or 24 months by default, with no warning), and the portal shows a *Secret ID* next to the *Value* — the Value is visible **once**, right after you create it, and looks no more secret than the ID. |
| **`invalid_grant`** | The `redirect_uri` differs between the two calls, or the authorization code was already used (reloading the callback page does this). |
| **`unauthorized_client`** | The app registration does not permit this flow. |

**A note on Google specifically.** A newly created Google OAuth app starts in **Testing**, where only
the test users you list may authorise — everyone else gets `Error 403: access_denied` with a warning
page carrying *your* name. Publishing it requires Google's verification, and the Gmail and Drive
scopes are **restricted**, which additionally requires an annual paid security assessment. **Plan for
that before promising your users those two connectors**; Calendar, Docs and Sheets do not need it.
...
```bash
git fetch --tags
git checkout v1.11.15       # pick a tag from github.com/NeoRebels/sqemes/releases
```
...
*2026-08-26 (SQEM-273/274) — connector failures now name the provider's error code, and this guide
says what each one means, including the Google Testing-mode wall that every operator hits.*
